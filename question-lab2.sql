...
SELECT gr.Наименование
FROM dbo.Группы as gr
INNER JOIN dbo.[Дисциплина в семестре] as disin ON disin.[ID группы] = gr.[ID группы]
GROUP BY gr.Наименование
HAVING count(DISTINCT disin.[ID дисциплины в семестре]) > 1
...
-- Вывести для каждого преподавателя количество человекоэкзаме-нов, принятых им.

SELECT pr.Имя, pr.Фамилия, pr.Отчество, count(zv.[ID студента]) as 'Количество принятых экзаменов'
FROM dbo.Преподаватели as pr
INNER JOIN dbo.[Дисциплина в семестре] as disin ON disin.[ID лектора] = pr.[ID преподавателя]
INNER JOIN dbo.[Зачетная ведомость] as zv ON zv.[ID дисциплины в семестре] = disin.[ID дисциплины в семестре]
GROUP BY pr.Имя, pr.Фамилия, pr.Отчество


-- Вывести в виде «рейтинга» ТОП-10 преподавателей по средней отметке в зачетной ведомости.

SELECT pr.Имя, pr.Фамилия, pr.Отчество, avg(zv.Отметка) as 'Cр. Отметка'
FROM dbo.Преподаватели as pr
INNER JOIN dbo.[Дисциплина в семестре] as disin ON disin.[ID лектора] = pr.[ID преподавателя]
INNER JOIN dbo.[Зачетная ведомость] as zv ON zv.[ID дисциплины в семестре] = disin.[ID дисциплины в семестре]
GROUP BY pr.Имя, pr.Фамилия, pr.Отчество
ORDER BY 'Cр. Отметка' DESC
--OFFSET 10 ROWS


-- Вывести для каждого преподавателя суммарное количество часов нагрузки в неделю.

SELECT pr.Имя, pr.Фамилия, pr.Отчество, sum(disin.[Количество часов в неделю])
FROM dbo.Преподаватели as pr
INNER JOIN dbo.[Дисциплина в семестре] as disin ON disin.[ID лектора] = pr.[ID преподавателя]
GROUP BY pr.Имя, pr.Фамилия, pr.Отчество


-- Вывести для каждого преподавателя суммарное количество часов нагрузки в неделю для студентов каждого института.

SELECT pr.Имя, pr.Фамилия, pr.Отчество, inst.Наименование ,sum(disin.[Количество часов в неделю])
FROM dbo.Преподаватели as pr
INNER JOIN dbo.[Дисциплина в семестре] as disin ON disin.[ID лектора] = pr.[ID преподавателя]
INNER JOIN dbo.Группы as gr ON gr.[ID группы] = disin.[ID группы]
INNER JOIN dbo.Институты as inst ON inst.[ID института] = gr.[ID института]
GROUP BY pr.Имя, pr.Фамилия, pr.Отчество, inst.Наименование


-- По каждому институту вывести количество студентов (персон).

SELECT inst.Наименование, count(stgr.[ID студента]) as 'Количество студентов'
FROM dbo.Институты as inst
INNER JOIN dbo.Группы as gr ON gr.[ID института] = inst.[ID института]
INNER JOIN dbo.[Студенты в группах] as stgr ON stgr.[ID группы] = gr.[ID группы]
GROUP BY inst.Наименование


-- По каждому институту вывести количество преподавателей.

SELECT inst.Наименование, count(pr.[ID преподавателя]) as 'Количество преподавателей'
FROM dbo.Институты as inst
INNER JOIN dbo.Преподаватели as pr ON pr.[ID института] = inst.[ID института]
GROUP BY inst.Наименование
